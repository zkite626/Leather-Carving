...
export default function NotificationsPage() {
  ...
  useEffect(() => {
    ...
    fetchNotifications(1).then((res) => {
      setHasMore(1 < (res.pagination?.totalPages ?? 1));
    });
  ...
  const filtered = filter === 'unread' ? safeNotifications.filter((n) => !n.isRead) : safeNotifications;

  return (
    <div className={styles.page}>
      <section className={styles.hero}>
        <h1 className={styles.heroTitle}>通知中心</h1>
      </section>

      <div className={styles.container}>
        <div className={styles.header}>
          <span className={styles.title}>
            {filter === 'unread' ? '未读通知' : '全部通知'}
          </span>
